...
\PassOptionsToPackage{unicode,colorlinks,linktoc=all,bookmarks=true,CJKbookmarks=true}{hyperref}
...
\ifPDFTeX\else
  % xetex/luatex font selection
    ...
    \fi
    \setmonofont[,RawFeature={fallback=monofontfallback}]{Fira Code}
  \ifXeTeX
    \usepackage{xeCJK}
...
\section{Minecraft游戏入门\&GTIIT服务器介绍}\label{minecraftgtiit}
...
鉴于阅读这个教程的人很有可能是大一新生，有可能因为学业压力或者家庭原因，在上大学之前并没有过多的使用或深入使用过电脑，或者没有深入游玩Minecraft
Java版的经验，所以本教程将会尽量以0基础的前提进行创作，竭尽所能帮助新玩家学习和学会学习。
...
\includegraphics{Pictures/maodiechongni.png}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{目录}\label{section-1}

\begin{itemize}
\item
  \hyperref[minecraft]{Minecraft游戏通识}

  \begin{itemize}
  \item
    \hyperref[section-2]{互联网学习所需要的良好品德/品质}
  \item
    \hyperref[minecraft-1]{Minecraft的实用网站}
  \item
    \hyperref[minecraft-2]{进入正题，如何安装、运行Minecraft}

    \begin{itemize}
    \tightlist
    \item
      \hyperref[minecraftjava]{运行Minecraft的必要条件：Java运行环境，启动器}
    \item
      \hyperref[minecraft-3]{启动器：启动minecraft的程序}
    \item
      \hyperref[pcl]{PCL下载}
    \end{itemize}
  \item
    \hyperref[section-3]{正版？盗版？}
  \item
    \hyperref[section-4]{模组的安装}
  \item
    \hyperref[section-5]{整合包的安装}
  \item
    \hyperref[section-6]{光影、材质}
  \item
    \hyperref[mc]{如何购买正版MC}
  \end{itemize}
\item
  \hyperref[section-7]{生电服务器}

  \begin{itemize}
  \tightlist
  \item
    \hyperref[section-8]{什么是生电服务器？}
  \item
    \hyperref[section-9]{生电服务器的常用模组}
  \item
    \hyperref[section-10]{为什么？}
  \end{itemize}
\item
  \hyperref[section-11]{本校服务器}

  \begin{itemize}
  \tightlist
  \item
    \hyperref[section-12]{本校有什么服务器？}
  \item
    \hyperref[section-13]{如何加入服务器？}
  \item
    \hyperref[section-14]{生电服务器玩家习惯法}
  \end{itemize}
\item
  \hyperref[section-15]{如何进入我服}

  \begin{itemize}
  \item
    \hyperref[java]{Java的安装}

    \begin{itemize}
    \tightlist
    \item
      \hyperref[java-1]{如何选择Java版本}
    \item
      \hyperref[launcherx]{对于LauncherX启动器}
    \end{itemize}
  \item
    \hyperref[section-16]{我服基本信息}

    \begin{itemize}
    \tightlist
    \item
      \hyperref[section-17]{版本等基本信息}
    \item
      \hyperref[section-18]{连接方式}
    \end{itemize}
  \item
    \hyperref[section-19]{启动器选择和整合包的安装}

    \begin{itemize}
    \tightlist
    \item
      \hyperref[section-20]{不建议使用的启动器}
    \item
      \hyperref[section-21]{整合包在哪里？}
    \item
      \hyperref[hmcl]{HMCL}
    \item
      \hyperref[pcl2]{PCL2}
  ...
  \end{itemize}
\item
  \hyperref[section-22]{服务器内部基本介绍}

  \begin{itemize}
  \item
    \hyperref[section-23]{末地工业区}

    \begin{itemize}
    \tightlist
    \item
      \hyperref[section-24]{末地工业区大致地理位置}
    \item
      \hyperref[section-25]{末地伪和平}
    \item
      \hyperref[section-26]{刷沙机（末地部分）}
    \item
      \hyperref[section-27]{小黑塔}
    \end{itemize}
  \item
    \hyperref[section-28]{主世界部分}

    \begin{itemize}
    \tightlist
    \item
      \hyperref[section-29]{大致建筑和设施地理位置}
    \item
      \hyperref[section-30]{主世界伪和平}
    \end{itemize}
  \end{itemize}
\item
  \hyperref[section-31]{常见问题}

  \begin{itemize}
  \tightlist
  \item
    \hyperref[section-32]{整合包相关}

    \begin{itemize}
    \tightlist
    \item
      \hyperref[section-33]{屏幕上会显示背包内容}
    \item
      \hyperref[section-34]{飞行有问题}
    \end{itemize}
  \end{itemize}
\item
  \hyperref[section-35]{跋（其实就是后记，为了装逼我写跋这么个古代用词）}
\end{itemize}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{Minecraft游戏通识}\label{minecraft}

本部分会介绍Minecraft游戏的通识，即通用知识，并不只是在加入本校服务器时适用，而是对于所有Minecraft
Java版适用。

\begin{flushright}(13247903)\end{flushright}

\subsubsection{互联网学习所需要的良好品德/品质}\label{section-2}
...
\subsubsection{Minecraft的实用网站}\label{minecraft-1}

下面推荐几个minecraft实用网站：

\begin{itemize}
\item
  Minecraft官方网站：\url{https://www.minecraft.net/zh-hans}
  ...
  \end{itemize}
\item
  Minecraft中文wiki：\url{https://zh.minecraft.wiki/}
...
\subsubsection{进入正题，如何安装、运行Minecraft}\label{minecraft-2}

\paragraph{运行Minecraft的必要条件：Java运行环境，启动器}\label{minecraftjava}

Java运行环境，即JRE（Java Runtime
Environment）是我的世界运行的基础，我的世界是由java语言编写的，自然也要有java语言运行的环境，并且这门语言也在不断的更新迭代之中，不同的我的世界版本可能会使用不同版本的java语言，自然也需要不同版本的java运行环境。
...
\tightlist
\item
  自\href{https://zh.minecraft.wiki/w/17w13a}{1.12}起，运行Minecraft的最低要求为Java
  8
\item
  自\href{https://zh.minecraft.wiki/w/21w19a}{1.17}起，运行Minecraft的最低要求为Java
  16
\item
  自\href{https://zh.minecraft.wiki/w/Java版1.18-pre2}{1.18}起，运行Minecraft的最低要求为Java
  17
\item
  自\href{https://zh.minecraft.wiki/w/24w14a}{1.20.5}起，运行Minecraft的最低要求为Java
  21，且操作系统须为64位
\end{itemize}
...
\tightlist
\item
  Java 8：\url{https://www.java.com/zh-CN/download/?locale=zh}
\item
  Java
  16、17、21似乎是因为原公司被收购的原因，下载地址有些不同：\url{https://www.oracle.com/java/technologies/downloads/}
\end{itemize}
...
\paragraph{启动器：启动minecraft的程序}\label{minecraft-3}

你可能会问：我们只下载了java，又下载启动器，那游戏本体去哪了？

其实现在的主流我的世界启动器都支持游戏的本体下载，甚至支持模组下载，只需要安装启动器，就可以在启动器内安装游戏本体。你可能会问这种在第三方的启动器里下载的是正版还是盗版？可以移步\hyperref[section-3]{正版？盗版？}这一部分阅读。

目前主流的启动器有：\\
Minecraft官方启动器：我的世界Mojang工作室官方的启动器，不能说是十全十美吧，但也能称得上是史中之史，别TM用，支持下载游戏，但是使用的是国外的下载源，给你等成棍木了都不一定能下载完。可以在下载器里面换皮肤（详见\hyperref[section-3]{正版？盗版？}），但是皮肤似乎也可以在Minecraft官网上更换上传，所以就连这最后的一点作用官方启动器也失去了......
...
\paragraph{PCL下载}\label{pcl}

PCL是由中国大佬\textbf{龙腾猫跃}（这个是真大佬）所编写的minecraft启动器，在爱发电平台上发布，进入爱发电网站搜索PCL就能找到下载页面
...
\includegraphics{Pictures/PCL_1.png}

可以看到下载页面有一堆东西，如果你要安装模组、整合包或者光影的话，请移步\hyperref[modsinstall]{模组的安装}和\hyperref[guangying]{光影、材质}这两章节进行阅读。

不安装，只玩原版的话，就可以点击下载了。

同时，强烈建议开启
设置→启动→默认版本隔离→隔离所有版本。不开启的话你的模组、存档等等会变得一团糟！

而在安装好java后，需要设置 设置→启动→游戏Java→自动选择合适的Java
。这样Java环境才算真正配置好。
...
\subsubsection{正版？盗版？}\label{section-3}
...
尽管考虑到购买正版需要花费89
RMB的``巨款''，我们仍然强烈建议，甚至严肃要求您购入正版，尽管目前服务器并没有开启正版验证（即目前允许离线登录玩家进入服务器）。原因如下：
...
\subsubsection{模组的安装}\label{section-4}
...
\includegraphics{Pictures/PCL_2.png}
...
\subsubsection{整合包的安装}\label{section-5}
...
有关我服整合包的安装，请移步\hyperref[section-19]{启动器选择和整合包的安装}查看。

\begin{flushright}(9DA62C42)\end{flushright}

\subsubsection{光影、材质}\label{section-6}
...
\subsubsection{如何购买正版MC}\label{mc}

详见：\url{https://zhuanlan.zhihu.com/p/393174208}

\begin{flushright}(9DA62C42)\end{flushright}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{生电服务器}\label{section-7}

\subsubsection{什么是生电服务器？}\label{section-8}

生电，Survival
Circuit，即生存电路，通常指在原版游戏中利用游戏特性制作的红石机器。而生电服务器，就是以大量制造这类机器为特点的服务器。
...
\subsubsection{生电服务器的常用模组}\label{section-9}
...
\subsubsection{为什么？}\label{section-10}
...
\subsection{本校服务器}\label{section-11}

\subsubsection{本校有什么服务器？}\label{section-12}
...
\item
  ...
  我的世界小游戏服务器，有很多经典的游戏，由隔壁汕大oolong\_tea同学开设，属于社会服务器，玩家基本为校外各路人士，所以注意信息甄别。\\
  进服可以联系oolong\_tea或者sharpOwO
\end{itemize}
...
\subsubsection{如何加入服务器？}\label{section-13}

见\hyperref[section-15]{9DA62C42编写的教程}
...
\subsubsection{生电服务器玩家习惯法}\label{section-14}
...
\subsection{如何进入我服}\label{section-15}

\subsubsection{Java的安装}\label{java}

\paragraph{如何选择Java版本}\label{java-1}
...
\paragraph{对于LauncherX启动器}\label{launcherx}
...
\subsubsection{我服基本信息}\label{section-16}

\paragraph{版本等基本信息}\label{section-17}

版本：1.20.1，模式：纯净生存。

\begin{flushright}(9DA62C42)\end{flushright}

\paragraph{连接方式}\label{section-18}
...
\subsubsection{启动器选择和整合包的安装}\label{section-19}

\paragraph{不建议使用的启动器}\label{section-20}

不建议使用官方启动器，LauncherX启动器，因为无法安装整合包，如果无需模组辅助可以考虑（不建议）。

推荐使用PCL2和HMCL，如何下载请参阅\hyperref[minecraft-3]{启动器：启动minecraft的程序}。

\begin{flushright}(9DA62C42)\end{flushright}

\paragraph{整合包在哪里？}\label{section-21}

整合包在群文件里，搜索1.20.1，下载``1.20.1生电整合包.zip''，并且记录下载位置，之后有用。

\begin{flushright}(9DA62C42)\end{flushright}

\paragraph{HMCL}\label{hmcl}
...
\subsection{服务器内部基本介绍}\label{section-22}

\subsubsection{末地工业区}\label{section-23}

\paragraph{末地工业区大致地理位置}\label{section-24}

\begin{itemize}
\tightlist
\item
  (100,\textasciitilde,0): 刷沙机（末地部分）
\item
  (326,\textasciitilde,0): 小黑塔
\item
  (119,\textasciitilde,-236): 树场
\item
  (377,\textasciitilde,-241): 苔藓骨粉机
\item
  (-146,\textasciitilde,-250): 刷石机
\item
  (-428,\textasciitilde,-423): 320熔炉组
\item
  (-189,\textasciitilde,-387): 农场
\item
  (19,\textasciitilde,-412): 交易所
\item
  (-24,\textasciitilde,-566): 刷铁机
\item
  (-36,\textasciitilde,-757): 袭击塔
\item
  (16,\textasciitilde,-1664): 末地伪和平
\end{itemize}

\includegraphics{Pictures/theendbasement.png}

\begin{flushright}(9DA62C42)\end{flushright}

\paragraph{末地伪和平}\label{section-25}

警告：(16,\textasciitilde,-1664) 方圆300格内 不得进入！会卡死！\\
开启末地伪和平："player PeaCe rejoin"\\
关闭末地伪和平："player PeaCe kill"(伪和平关闭后仍然不得进入该区域)

\begin{flushright}(9DA62C42)\end{flushright}

\paragraph{刷沙机（末地部分）}\label{section-26}

注意：刷沙机不可空跑，运行时需要保证假人Sand1, Sand2都在线。\\
...
\paragraph{小黑塔}\label{section-27}

小黑塔常用于获取经验，获得经验用于经验修补、附魔和铁砧，也可以获取末影珍珠。末影珍珠储存于箱子中，多余的会被投掷入岩浆中。

平时，为防止末影人刷新，末地会开启伪和平，利用指令"player PeaCe
kill"即可解除伪和平（假人科技）。使用完毕后记得开启伪和平，使用指令"player
PeaCe rejoin"开启。

\begin{flushright}(9DA62C42)\end{flushright}

\subsubsection{主世界部分}\label{section-28}

\paragraph{大致建筑和设施地理位置}\label{section-29}

\begin{itemize}
\tightlist
\item
  (55,\textasciitilde,4751): 主世界伪和平
\item
  (1339,\textasciitilde,704): 广以校徽
\item
  (1734,\textasciitilde,324): 史莱姆农场
\end{itemize}

\begin{flushright}(9DA62C42)\end{flushright}

\paragraph{主世界伪和平}\label{section-30}

警告：(55,\textasciitilde,4751) 方圆300格内 不得进入！会卡死！\\
...
\subsection{常见问题}\label{section-31}

\subsubsection{整合包相关}\label{section-32}

\paragraph{屏幕上会显示背包内容}\label{section-33}

此现象是物品栏HUD+导致，按"I"可以关闭。此模组的按键绑定可以在选项→按键控制→按键绑定→Inventory
HUD+中修改。

\begin{flushright}(9DA62C42)\end{flushright}

\paragraph{飞行有问题}\label{section-34}

如果发现鞘翅飞行有些奇怪，是因为Do a Barrel
Roll模组，在启动器中关闭模组或者删除即可。

\begin{flushright}(9DA62C42)\end{flushright}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{跋（其实就是后记，为了装逼我写跋这么个古代用词）}\label{section-35}
